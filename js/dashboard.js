...
class FranchiseDashboard {
  ...
  async loadRealData() {
    try {
      ...
      const response = await fetch(
        `https://api.jsonbin.io/v3/b/analytics-${today}`,
        {
          headers: {
  ...
  processRealAnalyticsData(analyticsData) {
    ...
    const successfulRoutes = analyticsData.filter(
      (d) => d.selectedLocationId !== null
    ).length;
    ...
    analyticsData.forEach((record) => {
      if (record.selectedLocation && record.selectedLocation !== "NONE") {
        ...
        if (record.distance) {
          locationStats[record.selectedLocation].distances.push(
            record.distance
          );
        }
      }
    ...
    return {
      summary: {
        ...
        responseTime: Math.round(
          analyticsData.reduce((sum, d) => sum + (d.responseTime || 20), 0) /
  ...
  async simulateAPICall() {
    ...
    const totalLeads = locationData.reduce(
      (sum, loc) => sum + loc.leadsToday,
      0
    );
    const avgConversion = Math.round(
      locationData.reduce((sum, loc) => sum + loc.conversionRate, 0) /
        locationData.length
    );
    const avgResponse = Math.round(
      locationData.reduce((sum, loc) => sum + loc.responseTime, 0) /
        locationData.length
    );
  ...
  updateUI() {
    ...
    document.getElementById("totalLeads").textContent =
      this.data.summary.totalLeads;
    document.getElementById(
      "conversionRate"
    ).textContent = `${this.data.summary.conversionRate}%`;
    document.getElementById(
      "responseTime"
    ).textContent = `${this.data.summary.responseTime}min`;
    document.getElementById("activeLocations").textContent =
      this.data.summary.activeLocations;

    // Update change indicators
    this.updateChange("leadsChange", this.data.summary.leadsChange, "%");
    this.updateChange(
      "conversionChange",
      this.data.summary.conversionChange,
      "%"
    );
    this.updateChange(
      "responseChange",
      this.data.summary.responseChange,
      "min"
    );
    ...
    document.getElementById("locationsChange").className =
    ...
    this.data.locations.forEach((location) => {
      ...
      row.innerHTML = `
                    ...
                    <div style="display: flex; align-items: center; padding: 5px 0;">
                        <div style="width: 10px; height: 10px; border-radius: 50%; background: ${
                          location.status === "active"
                            ...
                            ? "#f39c12"
                            : "#e74c3c"
                        }; margin-right: 12px; flex-shrink: 0;"></div>
                        <div style="min-width: 0; flex: 1;">
                            <div style="font-size: 0.95rem; font-weight: 600; color: var(--text-primary); margin-bottom: 2px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${
                              location.name
                            }</div>
                            <div style="color: var(--text-secondary); font-weight: 500; font-size: 0.8rem;">${
                              location.city
                            }, ${location.state}</div>
                        </div>
                    </div>
                </td>
                <td><div style="font-size: 1.05rem; font-weight: 700; color: var(--text-primary); text-align: center;">${
                  location.leadsToday
                }</div></td>
                <td><div style="font-weight: 600; color: ${
                  location.conversionRate > 25
                    ? "#27ae60"
                    : location.conversionRate > 15
                    ? "#f39c12"
                    : "#e74c3c"
        ...
        location.conversionRate
                    ...
                    <div style="padding: 5px 0;">
                        <div class="capacity-bar">
                            <div class="capacity-fill" style="width: ${
                              location.capacity
                            }%; background: ${capacityColor};"></div>
                        </div>
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            <small style="font-weight: 600; color: var(--text-secondary); font-size: 0.75rem;">${
                              location.capacity
                            }%</small>
                ...
                <td><div style="font-weight: 600; color: var(--text-primary); text-align: center; font-size: 0.95rem;">${
